...
# MAGIC %md
# MAGIC # Bronze — pokemon_abilities_raw (PokeAPI /ability)
# MAGIC Tal cual viene, sin transformar (reglas/01-datos-medallion.md).
...
urls = fetch_all_resource_urls(f"{base_url}/ability", limit=limit)
...
df.write.format("delta").mode("overwrite").saveAsTable(f"{catalog_name}.bronze.pokemon_abilities_raw")
