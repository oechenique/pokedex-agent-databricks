...
# MAGIC %md
# MAGIC # Bronze — pokemon_species_raw (PokeAPI /pokemon-species)
# MAGIC Flavor text, tasa de captura, cadena de evolución, etc. -- tal cual
# MAGIC viene, sin transformar (reglas/01-datos-medallion.md).
...
urls = fetch_all_resource_urls(f"{base_url}/pokemon-species", limit=limit)
...
df.write.format("delta").mode("overwrite").saveAsTable(f"{catalog_name}.bronze.pokemon_species_raw")
