...
# MAGIC %md
# MAGIC # Bronze — pokemon_api_raw (PokeAPI /pokemon)
# MAGIC Ingesta cruda: un notebook por fuente (reglas/01-datos-medallion.md).
# MAGIC El JSON de cada pokemon se guarda tal cual, sin transformar -- Silver
# MAGIC es quien aplana/tipa/normaliza, nunca este notebook.
...
urls = fetch_all_resource_urls(f"{base_url}/pokemon", limit=limit)
...
df.write.format("delta").mode("overwrite").saveAsTable(f"{catalog_name}.bronze.pokemon_api_raw")
